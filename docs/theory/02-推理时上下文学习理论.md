...
本文档介绍DAML-RAG框架中**推理时上下文学习（In-Context Learning, ICL）**的理论基础,纠正了之前"推理时元学习"的错误命名。核心机制结合Few-Shot学习、上下文注入和案例推理,使小模型在无需额外训练的情况下,在垂直领域获得专家级表现。
...
DAML-RAG的"推理时上下文学习"方法（之前错误地命名为"推理时元学习"）不是新的理论贡献,而是一种工程最佳实践,它：
