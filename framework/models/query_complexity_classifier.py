# -*- coding: utf-8 -*-
"""
QueryComplexityClassifier - 查询复杂度分类器（v4.0融合版）

基于BGE向量模型的语义相似度分类

v4.0 融合版：
- 集成v3.x完整功能（BGE-M3向量模型 + 硬编码关键词兜底）
- 适配v4.x框架接口体系
- 融入v4.x工程化特性（监控、日志）
...
class QueryComplexityClassifier:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(f"[QueryComplexityClassifier] 加载BGE模型: {self.model_name}...")
            self._model = SentenceTransformer(self.model_name)
            logger.info("[QueryComplexityClassifier] ✅ BGE模型加载成功")
